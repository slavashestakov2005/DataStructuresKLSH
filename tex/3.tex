\coursefooterdate{?.06.2024}\head{\Large Урок 3: Доработка Vector.}
\label{md2tex3}
...
Теперь реализуем операцию удаления из конца вектора: удаляем (уменьшаем \mintinline{cpp}{cur_size}) и если получилось, что \mintinline{cpp}{cur_size * 4 < cur_capacity}, то выделяем заново память размера \mintinline{cpp}{cur_size * 2} и копируем туда данные. Почему константы выбраны именно так, мы поговорим на следующем занятии.
